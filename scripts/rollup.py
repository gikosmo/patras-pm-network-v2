...
The window's end date defaults to TODAY (UTC), not yesterday -- run this
on 2026-09-16 with no --end and you get 2026-09-10 -> 2026-09-16; run it
again tomorrow and it becomes 2026-09-11 -> 2026-09-17, and so on. Today's
own day is still in progress when you run this, so its "expected slot"
count (for the completeness% calculation) is scaled to only the time
that's actually elapsed since midnight UTC -- see
analyze.expected_slots_for_date() -- otherwise every healthy sensor would
show ~50% completeness at midday and get incorrectly flagged WARNING.
...
from analyze import (
    pair_stats, classify, attach_nearest_neighbors, expected_slots_for_date,
    SIZE_FRACTIONS, PRIMARY_FRACTION, CONFIG,
)
...
def rollup(days: int = 7, end_date: str | None = None):
    ...
    # Sum per-day expected slots rather than a flat (full_day * n_days): a
    # window that includes today (still in progress) should only expect
    # today's elapsed slots, not a full day's worth.
    expected_slots_total = sum(expected_slots_for_date(d, avg_min) for d in window_dates)
