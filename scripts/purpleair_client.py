...
def get_sensors_in_bbox(
    bbox: dict, fields: Iterable[str] = SENSOR_LIST_FIELDS, max_age: int = 15552000
) -> list[dict]:
    """
    ...
    Kept for reference/backfill scripts; day-to-day fetching now uses
    get_sensors_by_ids() against the network's own station spreadsheet (see
    scripts/build_station_list.py) instead of a lat/long box, so sensors are
    never silently dropped for sitting just outside the box or for being
    registered as indoor (location_type).
    """
    params = {
        "fields": ",".join(fields),
        "nwlat": bbox["nwlat"],
...
def get_sensors_by_ids(
    sensor_ids: Iterable[int | str],
    fields: Iterable[str] = SENSOR_LIST_FIELDS,
    batch_size: int = 100,
    max_age: int = 15552000,
) -> list[dict]:
    """
    ...
    list) instead of a bounding box. This is PatrasAir's normal sensor
    selection: the station list comes from the network's own spreadsheet
    (data/patras_station_list.csv -> config.json's station_ids), so a
    sensor is included as long as it's on that list -- regardless of
    whether it sits inside any particular lat/long box, and regardless of
    whether PurpleAir has it registered as indoor or outdoor.

    max_age defaults to 15552000 seconds (180 days) rather than PurpleAir's
    own default of 604800 (7 days), so a sensor that's been offline for a
    while still comes back (correctly flagged OFFLINE by analyze.py)
    instead of silently vanishing from the results.

    Batches requests at `batch_size` ids per call to stay well within
    PurpleAir's URL-length / fair-use limits.
    """
...
def day_bounds_utc(date_str: str | None = None) -> tuple[int, int]:
    ...
    If date_str is today (UTC), end_ts is clamped to right now rather than
    23:59:59 -- the day is still in progress, and asking PurpleAir's
    history endpoint for timestamps in the future isn't meaningful.
    """
    if date_str is None:
